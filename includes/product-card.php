...
<div onclick='openQuickView(<?php echo $jsData; ?>)' class="zk-card cursor-pointer bg-white rounded-2xl overflow-hidden border border-gray-100/80 flex flex-col relative shadow-sm hover:shadow-2xl" style="box-shadow: 0 4px 15px -3px rgba(0,0,0,0.06);">
    <!-- Image -->
    <div class="overflow-hidden relative bg-slate-50" style="padding-top: 78%;">
        <img loading="lazy"
            src="<?php echo htmlspecialchars($imgSrc); ?>"
            alt="<?php echo htmlspecialchars($p['name']); ?>"
            class="zk-img absolute inset-0 w-full h-full object-cover"
            onerror="this.src='https://placehold.co/400x300/FFF7ED/F97316?text=Kitchen+Pro'">
        ...
        <div class="absolute top-3 right-3 z-20 flex flex-col gap-2">
            ...
            <button onclick="event.stopPropagation(); toggleCompare(<?php echo htmlspecialchars($jsData); ?>)" 
                class="w-8 h-8 rounded-xl bg-white/90 backdrop-blur border border-slate-100 flex items-center justify-center text-slate-400 hover:text-amber-500 hover:border-amber-200 shadow-sm transition" title="Compare">
    ...
    <div class="p-4 flex flex-col flex-1">
        <h3 class="font-black text-slate-900 text-sm leading-tight mb-3 line-clamp-2 group-hover:text-orange-600 transition" style="letter-spacing: -0.01em;">
        ...
        <div class="mt-auto pt-3 flex items-center justify-between border-t border-slate-50 gap-2">
            ...
            <div class="flex items-center gap-1.5 relative z-20">
                ...
                <button onclick="event.stopPropagation(); addToCart(<?php echo htmlspecialchars(json_encode($p)); ?>);"
                    class="h-9 w-9 flex-shrink-0 flex items-center justify-center rounded-xl bg-slate-50 hover:bg-orange-50 text-slate-500 hover:text-orange-500 border border-slate-100 hover:border-orange-200 transition group/btn shadow-sm">
